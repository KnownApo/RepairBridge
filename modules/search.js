...
async function loadLaborEstimatePanel(vinData) {
    ...
    try {
        ...
        const hoursText = estimate.laborHours !== undefined && estimate.laborHours !== null
...
function saveTsbFallback() {
    ...
    if (file) {
        ...
        if (file.size > maxBytes) {
            showNotification('TSB PDF is too large for local storage. Please use a link instead.', 'warning');
            if (!link) return;
            finalizeSave();
            return;
        }
