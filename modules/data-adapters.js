...
RepairBridgeDataAdapters.registerAdapter({
  ...
  getMakes: async ({ query } = {}) => {
    ...
    return list
      .filter((item) => item.Make_Name && item.Make_Name.toLowerCase().includes(q))
      .slice(0, 10);
  },
  getModelsForMake: async (make) => {
    if (!make) return [];
  ...
  getVehicleMatches: async ({ query, make }) => {
    ...
    return (filteredModels.length ? filteredModels : models).slice(0, 8).map((item) => ({
      vin: "—",
      make: resolvedMake,
...
RepairBridgeDataAdapters.registerAdapter({
  ...
  getModelsForMake: async (make) => {
    ...
    const models = mockVehicles
      .filter((item) => item.make.toLowerCase() === String(make).toLowerCase())
  ...
  getVehicleMatches: async ({ query, make }) => {
    ...
    if (q) {
      matches = matches.filter(
        (item) => item.make.toLowerCase().includes(q) || item.model.toLowerCase().includes(q)
      );
    }
