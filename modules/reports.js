...
function buildVinReportCsvLines(payload) {
    ...
    const vehicle = vehicleBase
        ? `${vehicleBase}${vinData?.trim ? ` (${vinData.trim})` : ''}`
        : '';
...
function renderSavedReports() {
    ...
    const reports = JSON.parse(localStorage.getItem('rb_saved_reports') || '[]');
    if (!reports.length) {
        container.innerHTML = '<div class="activity-item"><span>No saved reports yet</span><small>Download a VIN report to save it here</small></div>';
        return;
    }

    container.innerHTML = reports.map(report => {
        const tsbText = report.tsbs === null || report.tsbs === undefined ? 'TSBs: —' : `TSBs: ${report.tsbs}`;
        const savedAt = report.downloadedAt ? new Date(report.downloadedAt).toLocaleString() : 'Just now';
        return `
...
function renderSavedReportView(report) {
    ...
    if (subtitleEl) subtitleEl.textContent = report.downloadedAt
    ...
    renderList(details?.recalls || [], recallEl, details ? 'No recalls recorded' : 'Details unavailable. Re-run the report to refresh.');
    renderList(details?.complaints || [], complaintEl, details ? 'No complaints recorded' : 'Details unavailable. Re-run the report to refresh.');

    if (tsbEl) {
        if (!details) {
            tsbEl.innerHTML = '<li class="report-empty">Details unavailable. Re-run the report to refresh.</li>';
        } else if (details.tsbs === null) {
            tsbEl.innerHTML = '<li class="report-empty">TSB data unavailable (no free public endpoint).</li>';
        } else if (!details.tsbs.length) {
            tsbEl.innerHTML = '<li class="report-empty">No TSBs recorded</li>';
        } else {
            tsbEl.innerHTML = details.tsbs.map(item => `
                <li>
                    <strong>${item.component}</strong>
                    <span>${item.summary}</span>
                </li>
        ...
        if (fallbackItems.length) {
            const fallbackHtml = fallbackItems.map(item => {
                const linkHtml = item.link ? `<a href="${item.link}" target="_blank" rel="noopener">${item.link}</a>` : '';
    ...
    if (noteEl) {
        noteEl.textContent = 'Data is provided for informational purposes only. Verify before making repair decisions.';
    }
}
